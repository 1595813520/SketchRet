...
def parse_args():
    ...
    parser.add_argument('--fixed_prompt', type=str, default=None, help='Use one fixed prompt for every sample. If set, per-sample captions are ignored everywhere.')
...
'''
accelerate launch /data4/Sketch/sketch_retarget_controlnext_v2/train_v7_tr_coarseanchor.py \
  --pretrained_model_name_or_path /data4/model/foolkatGODOF_v3_diffusers \
  --crop_root /data4/turing_dataset/crop \
  --index_file /data4/turing_dataset/crop/panel_index.jsonl \
  --validation_index_file /data4/turing_dataset/crop/splits/test_ref.jsonl \
  --output_dir /data4/Sketch/output/train_v7_turing \
  --train_batch_size 4 \
  --resume_from_checkpoint /data4/Sketch/output/train_v7_fixedprompt/checkpoint-30000 \
  --fixed_prompt "Generate clean black-and-white manga line art from the provided sketch. Preserve the sketch composition, character layout, poses, and key visible content. The output must be monochrome line art only: black ink lines on a white background, with no color, no grayscale shading, no screentones, no painterly rendering, and no text, logos, or watermarks." \
  --max_train_steps 50000 \
  --caption_dropout_prob 0.0 \
  --sketch_dropout_prob 0.0 \
  --checkpointing_steps 10001 \
  --validation_every_n_steps 1000 \
  --num_validation_samples 16 \
  --max_refs_per_panel 5 \
  --controlnext_scale 0.25 \
  --text_attn_scale 0.35 \
  --phase_a_ratio 0.10 \
  --phase_b_ratio 0.15 \
